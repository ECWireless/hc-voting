...
contract('HCVoting', ([appManager, user]) => {
  ...
  beforeEach('deploy dao and app', async () => {
    ...
    const newAppInstanceReceipt = await dao.newAppInstance(
      hash('hc-voting.aragonpm.test'), // appId - Unique identifier for each app installed in the DAO; can be any bytes32 string in the tests.
      appBase.address, // appBase - Location of the app's base implementation.
      '0x', // initializePayload - Used to instantiate and initialize the proxy in the same call (if given a non-empty bytes string).
      false, // setDefault - Whether the app proxy is the default proxy.
      { from: appManager }
    )
    app = await HCVoting.at(deployedContract(newAppInstanceReceipt))
